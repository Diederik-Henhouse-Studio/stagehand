/**
 * MVP: Oirschot Bestuurlijke Informatie Monitor (Cloud Version)
 *
 * This version uses Browserbase cloud browsers instead of local Chrome
 * Required: BROWSERBASE_API_KEY and BROWSERBASE_PROJECT_ID environment variables
 */
...
// Same schemas as the local version
...
async function analyzeOirschotPortal(): Promise<AnalysisResult> {
  ...
  if (!process.env.BROWSERBASE_API_KEY || !process.env.BROWSERBASE_PROJECT_ID) {
    throw new Error(
      ...
      'Sign up for free at https://browserbase.com'
    );
  }

  console.log('🚀 Starting Oirschot Portal Analysis (AI-Powered)...\n');

  // Initialize Stagehand with Browserbase cloud browser
  ...
  try {
    ...
    let sections;
    try {
      sections = await stagehand.extract({
    ...
    let recentItems;
    try {
      recentItems = await stagehand.extract({
    ...
    let meetings;
    try {
      meetings = await stagehand.extract({
